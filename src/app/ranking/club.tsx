    ...
    <S.Wrap>
      ...
      {CLUB_TUBLE_ITEMS.map((item, idx) => (
        <S.TubleWrap key={idx} onClick={() => router.push(`/stock/${idx}`)}>
          ...
          <S.Text $color={palette.commit_3}>{item.coin.present} 코인</S.Text>
          <S.Text $color={palette.commit_5}>{item.coin.high} 코인</S.Text>
          <S.Text $color={"#B1B4B9"}>{item.coin.low} 코인</S.Text>
        </S.TubleWrap>
      ))}
    </S.Wrap>
  );
